			...
			<tr>
				...
				<td>
					...
					foreach ($idDeal as $id) {
						...
						$preSymbol[] = array("symbol" => $symbol, "type" => $dealNum['type'], "id" => $dealNum['id']);
					}
					$preSymbol = array_map(function($v) { return '' . implode(',', $v) . ''; }, $preSymbol);
					echo '<pre>';
					...
					foreach ($preSymbol as $arrSymb1) {
						$line = explode(',', $arrSymb1);
						$line_search = $line[0].','.$line[1];
